...
\begin{titlepage}
    ...
    \begin{center} 
        ...
        % Bottom of the page
        {\large Word Count: 18990 Words \\ \today\par}
        \vfill
    \end{center}
\end{titlepage}
...
\begin{figure}[H]
	\begin{Center}
		\includegraphics[width = 0.7\linewidth]{./media/ControlFunctional.pdf}
...
\begin{wrapfigure}[14]{r}[0pt]{0.45\textwidth}
    \centering
    \raisebox{-5pt}[\dimexpr\height-3\baselineskip\relax]{%
...
\begin{figure}[H]
	\begin{Center}
		\includegraphics[width = 0.7\linewidth]{./media/ControlFlowChart.pdf}
		\caption[Flowchart describing ESP32 Data Transmission Algorithm]{\protect\raggedright Flowchart describing ESP32 Data Transmission Algorithm}
...
\subsubsection{Evaluation of Image Processing}
The algorithm applied was simple, but adequate for our use. By filtering out poor 
results, it has a low false positive rate. But as it is purely colour based, glitches
do occur. Improvements that could be made include applying a noise filter for better
low light performance, or an edge detection filter to combine colour detection with 
edge detection. Attempts were made to do this using a 3x3 kernel filter and the Filter IP 
provided by Intel, but due to limited memory constraints (See Appendix 1 \& 2), it was not possible. If there
was more time, we would try to reduce the memory usage of other components, such as by 
switching to SPI communications or to reduce the size of the FIFOs in the design and the frame
buffer. 
...
\section{Appendix 1: FPGA Resource Usage Summary}
\begin{table}[H]
    \begin{tabular}{lllll}
                              &                      &      &  &  \\
 ...
 \section{Appendix 2: FPGA Resource Usage by Entity}
 \begin{figure}[H]
     \centering
     \includegraphics[width=\textwidth]{FPGAResourceUtilisation.pdf}
 \end{figure}
\end{appendices}

\section{Appendix 3: GitHub Repository Source Code}
\url{https://github.com/JosiahMendes/Engineering-Design-Project-MARS-Rover}
