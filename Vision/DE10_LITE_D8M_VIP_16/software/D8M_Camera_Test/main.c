...
void calcLoc (int arr[colour][coords]){
    char* result = malloc(70);
    for (int i = 0; i< colour; i++){
        float x, y;
        int xlength, ylength;
        xlength = arr[i][2] - arr[i][0];
        ylength =  arr[i][3] - arr[i][1];
 
        if(xlength > 200 || xlength < 10 || ylength > 100 || ylength < 10){
            sprintf(result +strlen(result), "0.0,0.0;");
        } else if (xlength/ylength > 2.5 || ylength/xlength > 2.5){
            sprintf(result +strlen(result), "0.0,0.0;");
        } else {
